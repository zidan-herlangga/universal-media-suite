(function () {
  ...
  var STORE = {
    id: {
      ...
      "chg.title": "Catatan Rilis",
      "chg.subtitle": "Fitur baru dan perbaikan untuk setiap versi.",
      "chg.new": "Fitur Baru",
      "chg.fix": "Perbaikan Bug",
      "chg.note": "Catatan",
      "chg.2.new.1": "Installer resmi berbasis Inno Setup: wizard memilih lokasi instalasi, shortcut Start Menu & Desktop (opsional), serta uninstaller bawaan.",
      "chg.2.new.2": "Konversi PDF→DOCX kini memiliki 2 mode: Mode Visual (tiap halaman dirender sebagai gambar — hasil 100% persis PDF) dan Mode Teks (Editable).",
      "chg.2.new.3": "Render PNG transparan (RGBA) pada konversi PDF→DOCX mode visual.",
      "chg.2.new.4": "Build Windows diubah ke mode onedir — aplikasi terbuka lebih cepat dan konsumsi RAM lebih ringan.",
      "chg.2.fix.1": "Media Downloader: paste 1 link YouTube kini hanya mengunduh 1 video, tidak lagi seluruh playlist (perbaikan noplaylist).",
      "chg.2.fix.2": "PDF→DOCX yang sebelumnya berantakan (teks/tabel bergeser) terselesaikan lewat Mode Visual.",
      "chg.1.note.1": "Rilis perdana: konversi foto & RAW, video resizer, ekstraktor audio, dokumen & PDF tools (konversi, gabung, pisah), dan media downloader untuk Windows, Linux, dan macOS.",
      ...
      "dmod.4.pdf": "PDF → DOCX menyediakan 2 mode: Mode Visual (render per halaman jadi gambar, hasil persis) & Mode Teks (pdf2docx). Gabung/pisah halaman memakai pypdf dengan sintaks rentang 1-5 atau 2,4,7.",
    ...
    en: {
      ...
      "hero.badge": "Official Release v2.0.0",
      "hero.title": "Universal Media & Document Suite",
      ...
      "chg.title": "Release Notes",
      "chg.subtitle": "New features and fixes for each version.",
      "chg.new": "New Features",
      "chg.fix": "Bug Fixes",
      "chg.note": "Note",
      "chg.2.new.1": "Official Inno Setup installer: wizard to choose the install location, Start Menu & Desktop shortcuts (optional), plus a built-in uninstaller.",
      "chg.2.new.2": "PDF→DOCX now has 2 modes: Visual Mode (every page rendered as an image — 100% faithful to the PDF) and Text (Editable) Mode.",
      "chg.2.new.3": "Transparent (RGBA) PNG rendering for visual PDF→DOCX conversion.",
      "chg.2.new.4": "Windows build switched to onedir mode — the app launches faster and uses less RAM.",
      "chg.2.fix.1": "Media Downloader: pasting 1 YouTube link now downloads only 1 video, not the whole playlist (noplaylist fix).",
      "chg.2.fix.2": "Messy PDF→DOCX output (shifting text/tables) solved via Visual Mode.",
      "chg.1.note.1": "Initial release: photo & RAW conversion, video resizer, audio extractor, document & PDF tools (convert, merge, split), and a media downloader for Windows, Linux, and macOS.",
      ...
      "dmod.4.pdf": "PDF → DOCX offers 2 modes: Visual Mode (render each page as an image, faithful output) & Text Mode (pdf2docx). Merge/split pages uses pypdf with range syntax 1-5 or 2,4,7.",
